...
setSelectedIndex(document.getElementById("ddl_units"), "miles");
//unit_handler
...
function getRoutePath() {
  ...
  var deleteNode = function(mev) {
    if (mev.vertex != null) {
      routePath.getPath().removeAt(mev.vertex);
      routePathpolygonChanged();
    }
  };
  google.maps.event.addListener(routePath, 'rightclick', deleteNode);
  //Right click to delete vertex
...
function clearMap() {
  ...
  //remove elevationmarker if it exists
  if (elevationmarker) {
...
function LoadPosandSettings() {
  ...
  if (loadedstring != "") {
    ...
    if (splitstr[6] != null) {
      ...
      setSelectedIndex(document.getElementById("ddl_units"), unit_handler.label);
    }
  }
}
...
function empty() {
  ...
  if (ta.getAttribute("class") == "notselected" && ta.value == "")
    ta.value = "Input in the format lat,lng e.g.\n48.857798,2.299962\n47.857798,2.299962";
  if (ta.getAttribute("class") == "selected")
    ta.value = "";
}
